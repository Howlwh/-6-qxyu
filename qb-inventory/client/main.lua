...
local function Coke()
	itemInfos = {
		[1] = {costs = QBCore.Shared.Items["bandnote"]["label"] .. ": 1x, "},
	}

	local items = {}
	for k, item in pairs(Config.Coke["items"]) do
		local itemInfo = QBCore.Shared.Items[item.name:lower()]
		items[item.slot] = {
			name = itemInfo["name"],
			amount = tonumber(item.amount),
...
local function GetCokeThresholdItems()
	Coke()
	local items = {}
	for k, item in pairs(Config.Coke["items"]) do
		if PlayerData.metadata["craftingrep"] >= Config.CraftingItems[k].threshold then
		items[k] = Config.Coke["items"][k]
	end
end
	return items
end
...
RegisterNetEvent('inventory:client:Coke', function(itemName, itemCosts, amount, toSlot, points)
    local ped = PlayerPedId()
    SendNUIMessage({
        action = "close",
    })
        TriggerServerEvent("inventory:server:Coke", itemName, itemCosts, amount, toSlot, points)
        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[itemName], 'add')
        isCrafting = false
	end, function() -- Cancel
        QBCore.Functions.Notify("Failed", "error")
...
CreateThread(function()
    ...
    RegisterNetEvent("inventory:client:Coke", function(dropId)
        local crafting = {}
        crafting.label = "Coke"
        crafting.items = GetCokeThresholdItems()
        TriggerServerEvent("inventory:server:OpenInventory", "Coke", math.random(1, 99), crafting)
    end)
