...
#ifndef EDITMXAO_MIPLEVEL_AO
 #define EDITMXAO_MIPLEVEL_AO		0	//[0 to 2]      Miplevel of AO texture. 0 = fullscreen, 1 = 1/2 screen width/height, 2 = 1/4 screen width/height and so forth. Best results: IL MipLevel = AO MipLevel + 2
#endif

#ifndef EDITMXAO_MIPLEVEL_IL
 #define EDITMXAO_MIPLEVEL_IL		2	//[0 to 4]      Miplevel of IL texture. 0 = fullscreen, 1 = 1/2 screen width/height, 2 = 1/4 screen width/height and so forth.
#endif

#ifndef EDITMXAO_ENABLE_IL
 #define EDITMXAO_ENABLE_IL			1	//[0 or 1]	    Enables Indirect Lighting calculation. Will cause a major fps hit.
#endif

#ifndef EDITMXAO_SMOOTHNORMALS
 #define EDITMXAO_SMOOTHNORMALS     0   //[0 or 1]      This feature makes low poly surfaces smoother, especially useful on older games.
#endif

#ifndef EDITMXAO_TWO_LAYER
 #define EDITMXAO_TWO_LAYER         0   //[0 or 1]      Splits MXAO into two separate layers that allow for both large and fine AO.
#endif

#ifndef EDITMXAO_HIGH_QUALITY
 #define EDITMXAO_HIGH_QUALITY      0   //[0 or 1]      Enables a different, more physically accurate but slower SSAO mode. Based on Ground Truth Ambient Occlusion by Activision. No IL yet.
#endif
...
uniform int EDITMXAO_GLOBAL_SAMPLE_QUALITY_PRESET <
	ui_type = "combo";
...
uniform float EDITMXAO_SAMPLE_RADIUS <
	ui_type = "drag";
...
> = 2.5;
...
#if (EDITMXAO_HIGH_QUALITY==0)
    uniform float EDITMXAO_SAMPLE_NORMAL_BIAS <
        ui_type = "drag";
    ...
    > = 0.2;
#else
    #define EDITMXAO_SAMPLE_NORMAL_BIAS 0       //don't break PS which needs this, cleaner this way
#endif

uniform float EDITMXAO_GLOBAL_RENDER_SCALE <
	ui_type = "drag";
...
uniform float EDITMXAO_SSAO_AMOUNT <
	ui_type = "drag";
...
#if(EDITMXAO_ENABLE_IL != 0)
uniform float EDITMXAO_SSIL_AMOUNT <
    ui_type = "drag";
...
uniform float EDITMXAO_SSIL_SATURATION <
    ui_type = "drag";
...
> = 1.00;
...
#if (EDITMXAO_TWO_LAYER != 0)
    uniform float EDITMXAO_SAMPLE_RADIUS_SECONDARY <
        ui_type = "drag";
    ...
    uniform float EDITMXAO_AMOUNT_FINE <
        ui_type = "drag";
    ...
    uniform float EDITMXAO_AMOUNT_COARSE <
        ui_type = "drag";
        ui_min = 0.00; ui_max = 1.00;
...
uniform int EDITMXAO_BLEND_TYPE <
	ui_type = "slider";
	ui_min = 0; ui_max = 3;
...
uniform float EDITMXAO_FADE_DEPTH_START <
	ui_type = "drag";
...
uniform float EDITMXAO_FADE_DEPTH_END <
	ui_type = "drag";
...
uniform int EDITMXAO_DEBUG_VIEW_ENABLE <
	ui_type = "combo";
...
texture2D EDITMXAO_ColorTex 	{ Width = BUFFER_WIDTH;   Height = BUFFER_HEIGHT;   Format = RGBA8; MipLevels = 3+EDITMXAO_MIPLEVEL_IL;};
texture2D EDITMXAO_DepthTex 	{ Width = BUFFER_WIDTH;   Height = BUFFER_HEIGHT;   Format = R16F;  MipLevels = 3+EDITMXAO_MIPLEVEL_AO;};
texture2D EDITMXAO_NormalTex	{ Width = BUFFER_WIDTH;   Height = BUFFER_HEIGHT;   Format = RGBA8; MipLevels = 3+EDITMXAO_MIPLEVEL_IL;};

sampler2D sEDITMXAO_ColorTex	{ Texture = EDITMXAO_ColorTex;	};
sampler2D sEDITMXAO_DepthTex	{ Texture = EDITMXAO_DepthTex;	};
sampler2D sEDITMXAO_NormalTex	{ Texture = EDITMXAO_NormalTex;	};
...
EDITMXAO_VSOUT VS_MXAO(in uint id : SV_VertexID)
{
    EDITMXAO_VSOUT MXAO;

    MXAO.uv.x = (id == 2) ? 2.0 : 0.0;
    MXAO.uv.y = (id == 1) ? 2.0 : 0.0;
    MXAO.uv.zw = MXAO.uv.xy / EDITMXAO_GLOBAL_RENDER_SCALE;
    ...
    MXAO.samples   = samples_per_preset[EDITMXAO_GLOBAL_SAMPLE_QUALITY_PRESET];
...
float3 get_position_from_uv(in float2 uv, in EDITMXAO_VSOUT MXAO)
{
    return (uv.xyx * MXAO.uvtoviewMUL + MXAO.uvtoviewADD) * qUINT::linear_depth(uv) * RESHADE_DEPTH_LINEARIZATION_FAR_PLANE;
}

float3 get_position_from_uv_mipmapped(in float2 uv, in EDITMXAO_VSOUT MXAO, in int miplevel)
{
    return (uv.xyx * MXAO.uvtoviewMUL + MXAO.uvtoviewADD) * tex2Dlod(sEDITMXAO_DepthTex, float4(uv.xyx, miplevel)).x;
}
...
void spatial_blur_data(inout BlurData o, in sampler inputsampler, in float inputscale, in float4 uv)
{
	...
	o.mask = tex2Dlod(sEDITMXAO_NormalTex, uv);
...
float4 blur_filter(in EDITMXAO_VSOUT MXAO, in sampler inputsampler, in float inputscale, in float radius, in int blursteps)
{
...
void sample_parameter_setup(in EDITMXAO_VSOUT MXAO, in float scaled_depth, in float layer_id, out float scaled_radius, out float falloff_factor)
{
    scaled_radius  = 0.25 * EDITMXAO_SAMPLE_RADIUS / (MXAO.samples * (scaled_depth + 2.0));
    falloff_factor = -1.0/(EDITMXAO_SAMPLE_RADIUS * EDITMXAO_SAMPLE_RADIUS);

    #if(EDITMXAO_TWO_LAYER != 0)
        scaled_radius  *= lerp(1.0, EDITMXAO_SAMPLE_RADIUS_SECONDARY + 1e-6, layer_id);
        falloff_factor *= lerp(1.0, 1.0 / (EDITMXAO_SAMPLE_RADIUS_SECONDARY * EDITMXAO_SAMPLE_RADIUS_SECONDARY + 1e-6), layer_id);
    #endif
}

void smooth_normals(inout float3 normal, in float3 position, in EDITMXAO_VSOUT MXAO)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        for(int direction_step = 1; direction_step <= 5; direction_step++)
        {
            ...
            float3 temp_normal = tex2Dlod(sEDITMXAO_NormalTex, float4(tap_uv, 0, 0)).xyz * 2.0 - 1.0;
...
void PS_InputBufferSetup(in EDITMXAO_VSOUT MXAO, out float4 color : SV_Target0, out float4 depth : SV_Target1, out float4 normal : SV_Target2)
{
...
void PS_StencilSetup(in EDITMXAO_VSOUT MXAO, out float4 color : SV_Target0)
{        
    if(    qUINT::linear_depth(MXAO.uv.zw) >= EDITMXAO_FADE_DEPTH_END
        || 0.25 * 0.5 * EDITMXAO_SAMPLE_RADIUS / (tex2D(sEDITMXAO_DepthTex, MXAO.uv.zw).x + 2.0) * BUFFER_HEIGHT < 1.0
        || MXAO.uv.z > 1.0
        || MXAO.uv.w > 1.0
        ) discard;
...
void PS_AmbientObscurance(in EDITMXAO_VSOUT MXAO, out float4 color : SV_Target0)
{
    ...
    if(EDITMXAO_GLOBAL_SAMPLE_QUALITY_PRESET == 7) MXAO.samples = 2 + floor(0.05 * EDITMXAO_SAMPLE_RADIUS / linear_depth);
    ...
    {                    
    	...
    	float3 delta_v = -position + get_position_from_uv_mipmapped(tap_uv, MXAO, sample_mip + EDITMXAO_MIPLEVEL_AO);                
        float v2 = dot(delta_v, delta_v);
        float vn = dot(delta_v, normal) * rsqrt(v2);

        float sample_ao = saturate(1.0 + falloff_factor * v2) * saturate(vn - EDITMXAO_SAMPLE_NORMAL_BIAS);
#if(EDITMXAO_ENABLE_IL != 0)
        [branch]
        if(sample_ao > 0.1)
        {
                float3 sample_il = tex2Dlod(sEDITMXAO_ColorTex, float4(tap_uv, 0, sample_mip + EDITMXAO_MIPLEVEL_IL)).xyz;
                float3 sample_normal = tex2Dlod(sEDITMXAO_NormalTex, float4(tap_uv, 0, sample_mip + EDITMXAO_MIPLEVEL_IL)).xyz * 2.0 - 1.0;
    ...
    color = saturate(color / ((1.0 - EDITMXAO_SAMPLE_NORMAL_BIAS) * MXAO.samples) * 2.0);
    color = color.BLUR_COMP_SWIZZLE;
    color = sqrt(color);

#if(EDITMXAO_TWO_LAYER != 0)
    color *= lerp(EDITMXAO_AMOUNT_COARSE, EDITMXAO_AMOUNT_FINE, layer_id); 
#endif
}

void PS_AmbientObscuranceHQ(in EDITMXAO_VSOUT MXAO, out float4 color : SV_Target0)
{
	float3 position = get_position_from_uv_mipmapped(MXAO.uv.zw, MXAO, 0);
	float3 normal 	= normalize(tex2D(sEDITMXAO_NormalTex, MXAO.uv.zw).xyz * 2.0 - 1.0); //fixes black lines	
	...
	float scaled_radius = EDITMXAO_SAMPLE_RADIUS / position.z / stepshalf * RESHADE_DEPTH_LINEARIZATION_FAR_PLANE;
	float falloff_factor = 0.25 * rcp(EDITMXAO_SAMPLE_RADIUS * EDITMXAO_SAMPLE_RADIUS);	
	...
	for(float i = 0; i < directions; i++)
	{
		...
		for(int j = 0; j < stepshalf; j++)
		{
			...
			float3 delta_v[2];
			delta_v[0] = -position + get_position_from_uv_mipmapped(tap_uv.xy, MXAO, sample_mip + EDITMXAO_MIPLEVEL_AO);  
			delta_v[1] = -position + get_position_from_uv_mipmapped(tap_uv.zw, MXAO, sample_mip + EDITMXAO_MIPLEVEL_AO); 
			...
			if(falloff.x < 0.8)
			{
				sample_il[0] = tex2Dlod(sEDITMXAO_ColorTex, float4(tap_uv.xy, 0, sample_mip + EDITMXAO_MIPLEVEL_IL)).xyz;
				sample_normal[0] = tex2Dlod(sEDITMXAO_NormalTex, float4(tap_uv.xy, 0, sample_mip + EDITMXAO_MIPLEVEL_IL)).xyz * 2.0 - 1.0;
			...
			if(falloff.y < 0.8)
			{
	            sample_il[1] = tex2Dlod(sEDITMXAO_ColorTex, float4(tap_uv.zw, 0, sample_mip + EDITMXAO_MIPLEVEL_IL)).xyz;
	            sample_normal[1] = tex2Dlod(sEDITMXAO_NormalTex, float4(tap_uv.zw, 0, sample_mip + EDITMXAO_MIPLEVEL_IL)).xyz * 2.0 - 1.0;       
...
void PS_SpatialFilter1(in EDITMXAO_VSOUT MXAO, out float4 color : SV_Target0)
{
    color = blur_filter(MXAO, sCommonTex0, EDITMXAO_GLOBAL_RENDER_SCALE, 0.75, 4);
}

void PS_SpatialFilter2(EDITMXAO_VSOUT MXAO, out float4 color : SV_Target0)
{
    float4 ssil_ssao = blur_filter(MXAO, sCommonTex1, 1, 1.0 / EDITMXAO_GLOBAL_RENDER_SCALE, 8);
    ssil_ssao *= ssil_ssao;
	color = tex2D(sEDITMXAO_ColorTex, MXAO.uv.xy);
...
#if(EDITMXAO_ENABLE_IL != 0)
	ssil_ssao.xyz  = lerp(dot(ssil_ssao.xyz, lumcoeff), ssil_ssao.xyz, EDITMXAO_SSIL_SATURATION) * EDITMXAO_SSIL_AMOUNT * 2.0;
#else
    ssil_ssao.xyz = 0.0;
#endif

#if(EDITMXAO_HIGH_QUALITY == 0)
	ssil_ssao.w  = 1.0 - pow(saturate(1.0 - ssil_ssao.w), EDITMXAO_SSAO_AMOUNT * 2.0);
#else
    ssil_ssao.w  = 1.0 - pow(saturate(1.0 - ssil_ssao.w), EDITMXAO_SSAO_AMOUNT);
#endif
    ssil_ssao    *= 1.0 - smoothstep(EDITMXAO_FADE_DEPTH_START, EDITMXAO_FADE_DEPTH_END, scenedepth * float4(2.0, 2.0, 2.0, 1.0));

    if(EDITMXAO_BLEND_TYPE == 0)
    {
        color.rgb -= (ssil_ssao.www - ssil_ssao.xyz) * blendfact * color.rgb;
    }
    else if(EDITMXAO_BLEND_TYPE == 1)
    {
        color.rgb = color.rgb * saturate(1.0 - ssil_ssao.www * blendfact * 1.2) + ssil_ssao.xyz * blendfact * colorgray * 2.0;
    }
    else if(EDITMXAO_BLEND_TYPE == 2)
    {
    ...
    if(EDITMXAO_DEBUG_VIEW_ENABLE == 1)
    {
        color.rgb = max(0.0, 1.0 - ssil_ssao.www + ssil_ssao.xyz);
        color.rgb *= (EDITMXAO_ENABLE_IL != 0) ? 0.5 : 1.0;
    }
    else if(EDITMXAO_DEBUG_VIEW_ENABLE == 2)
    {      
        color.rgb = tex2D(sEDITMXAO_NormalTex, MXAO.uv.xy).xyz;
               ...
               "\nMXAO is written by Marty McFly / Pascal Gilcher"; >
{
    pass
	{
		...
		RenderTarget0 = EDITMXAO_ColorTex;
		RenderTarget1 = EDITMXAO_DepthTex;
		RenderTarget2 = EDITMXAO_NormalTex;
	}
    pass
    {
        VertexShader = VS_MXAO;
